...
数据（data）： 是符号、数字、字母或者二进制序列，数据本身可能没有意义。
```
11110000100111111001001110001000
```
这只是比特的排列，但没有赋予特定的解释。

结构（structure）：是数据的组织方式，决定了数据的可用性

```
11110000 10011111 10010011 10001000
```
如果按照UTF-8来解读，它可能是某个emoji

信息（infomation）：是数据在某种结构下被解读后的可用内容。
```
11110000 10011111 10010011 10001000 -> 🐸
```
## 表（list）思维里的结构
说到数据结构的基本组成，Knuth告诉我们有两种，一种是线性表，一种则是树。

回想一下我可不可以回溯的找到一种原因去解释为什么是这两者，线性表是一种线性结构，
而树是一种非线性结构。似乎在名字里给出了我们一些线索**线性**。计算机从图灵机开始就是
假设一种在纸带上运作的读写头，这整个纸带就是一个表（list）似乎所有的计算机操作都
离不开这样一种假设，表结构从根本上与计算这个概念契合。

表结构在很多情况下可以最自然的表示顺序关系。在自然种，前后关系几乎是不可避免的，任何
一组元素，只要他们聚集在一起，都会存在一种顺序，这时候就可以认为他们构成了一个表结构。
小到自然数的构成。

表结构这种强调前后的位置关系的结构的引入也带来一种特性，局部性原理，cache的出现就是根据
如此原理。这也是表结构带来的特性。

## 树（tree）关系里的结构
虽然线性结构也可以通过一个元素和周围元素的距离代表他们之间的关系，但远远没有树这种数据结构
直观。这篇文章会聚焦在线性结构，树会在之后.


## 评价数据结构
评价一种数据结构，大概可以从其静态和动态性质，结构化数据的存储和表示，创建，修改，访问和销毁结构化数据的算法,
这些层面观察。

## 线性表（linear list）
线性表, n>=0个结点的序列X\[1\],X\[2\],...,X\[n\],

它的基本结构性质**仅是各项排成一行的相对位置**。

在这种结构里，如果n大于0则 x1是第一个元素，xn是最后一个元素；
如果1 k n，则第k个结点xk之前的是xk-1,之后的是xk+1.

线性表的操作
1. 访问表的第k个元素，查看或者改变其内容
2. 在第k个结点的前或后插入一个新结点
3. 删除第k个结点
4. 把两个或多个线性表合并为一个表
5. 把一个线性表划分为两个或者多个表
...
7. 确定表中的结点数
8. 根据结点中的值，排序表中结点
9. 搜索线性表，找出一个在某个具有特定值的结点

---

之后的内容会通过一系列的leetcode题目来培养和线性表的感觉。

--- 

## [21 Merge Two Sorted Lists （合并两个有序链表）](https://leetcode.com/problems/merge-two-sorted-lists/description/)

这题比较简单。我先给一个常规解法然后在给出一个可以优化的方案
...
struct ListNode *mergeTwoLists(struct ListNode *list1, struct ListNode *list2)
{
        ...
        struct ListNode *list_big = list1;
        struct ListNode *list_small = list2;

        if (list1->val < list2->val) {
                list_big = list2;
                list_small = list1;
        }
        for (struct ListNode *iter = list_big; iter != NULL;) {
                struct ListNode *next_iter = iter->next;
                for (struct ListNode *inner_iter = list_small;
                     inner_iter != NULL; inner_iter = inner_iter->next) {
                        struct ListNode *inner_next = inner_iter->next;
                        if (iter->val >= inner_iter->val) {
                                if (!inner_next ||
                                    (iter->val < inner_next->val)) {
                                        inner_iter->next = iter;
                                        iter->next = inner_next;
                                }
                        }
                }
                iter = next_iter;
...
```
两个有序的线性表合并，没有什么复杂的逻辑按理说30-40min应该可以想出一个跑的通的方案。
记录一下我的思路，既然需要合并两个表，肯定需要遍历两个表的所有元素。使用一个嵌套的
for-each就可以一个不落的遍历所有表中元素了，这里有一个限制，这里提供的表只有它的下一个
位置的信息，也就是它是一个单向链。

我的基本思路是，确定一个base表，另一个表负责提供元素添加到这个base表里。这样只需要考虑
从元素表中拿到元素试着插入base表里。

由于单向表限制，我没办法操作一个元素的前一个元素，
又因为2个表都是有序线性表，所以两张表中的元素有单调性，我可以从第1个元素就判断如何合并
两个表。我总是从相对大的表向小的表合并。

---

这是我在leetcode搜到的另一个写法，明显比我写的更加优雅简洁。我再看之前先问几个问题。
1. 这个代码怎么处理有序单向表导致没法访问前一个元素的问题？
2. 这个代码怎么确定合并顺序的就是大合并小还是小合并大？
```c
//使用dummy结点
struct ListNode* mergeTwoLists(struct ListNode* list1, struct ListNode* list2) {
...
```
首先这位提供了叫做dummy的空结点当作输出。他这个的核心逻辑很像我们找一个空白纸，
然后一起从头数给定的两个数列谁比较小就先把那个node写在纸上。直到跑完所有结点。

回答一下问题，
* 问题1,他不需要处理访问上一个元素的情况，他只需要1遍pass就可以处理所有数据
* 问题2，他的合并顺序是小的先合并。

这个方法的确减少了很多代码复杂性，看起来也更顺畅。值的学习。

## [32 merge K lists](https://leetcode.com/problems/merge-k-sorted-lists/description/)
```c
struct ListNode {
        int val;
        struct ListNode *next;
...
```
因为刚刚我们才做过2个有序表的合并，所以自然而然的就会想到多个有序表的合并可以通过2个
有序表合并来实做。代码很简单，没什么可以聊。

但是。

很明显这个解法是有问题的。在一个巨大lists的test case 会出现超时问题。
<details>
<summary> 详情
</summary>
...
我们来分析一下超时问题，大概就是调用`mergeTwoLists`的调用成本和这个函数有调用成本加上这个函数的时间成本 
$ \mathcal {O}(N \times M)$
发现问题了吧，N和M会因为数列的膨胀而变得很大，相当于重复遍历了好几遍非常大的node数列才导致超时的。
所以mergeKLists的时间复杂度是$ \mathcal {O}(K \times N \times M)$
如果K巨大，而且这个K会线性递增。所以mergeTwoLists的方案不能直接写在这个上面。需要别的思路。

思路也很简单，上面我们碰到的问题在于每一次合并两个数列之后都会加长需要遍历的数列，这样mergeTwoLists的参数list1就会越来越长。如果我可以把所有
结点都放在一个容器里，这个容器可以提供一个当前最小的结点，这样只需要放所有节点进入容器，然后取出最小节点，记录，直到容器内节点被取完。

这样相当于只需要遍历所有节点两遍。

当然存在这种容器，它叫做“最小堆”这是一种自平衡二叉树，之后在说树这个结构的时候会详细说它的，今天只会给一个用法和实现。
...
## [单链表的倒数第k个节点](https://leetcode.cn/problems/kth-node-from-end-of-list-lcci/description/)

这个比较简单，就是简单的，双指针的使用，这个逻辑很好想拉，前一个指针为了探路，后一个指针隔着一个k的距离，第一个指针碰到NULL之后就表示链表结束
那么刚好后一个指针就是从后往前数的第k个。

如下是[leetcode 19](https://leetcode.com/problems/remove-nth-node-from-end-of-list/submissions/)的代码
```c
struct ListNode {
        int val;
        struct ListNode *next;
...
struct ListNode *removeNthFromEnd(struct ListNode *head, int n)
{
        ...
        struct ListNode *del_node = kthp->next;
        if (del_node) {
                kthp->next = del_node->next;
                del_node->next = NULL;
        }
        return head;
}
...
```
同样的我们看一下这个函数的时间复杂度只有一个循环所以是 $ \mathcal{O}(N) $
和需要处理的链表的长度有关。这个比较粗糙的版本有一些问题，比如说这个循环里的条件句是不是
可以不用。我找了一个更简洁的代码，我们一起看一下
...
这个代码处理的就很漂亮，结构化的看一下，删除倒数k个节点会面临什么问题。

1. 输入的值等于列表长度的情况，这时候就是删除第一个节点。
2. 启动第二个指针的时机怎么把握。
3. 怎么删除目标节点。

这个代码把一个循环拆分成两个循环，这样可以避免循环内的条件判断。最后删除的代码
`second->next->next` 没有问题是因为第二个指针不会比第一个指针更接近NULL，所以当
第一个指针的next是NULL的时候第二个指针的下一个的下一个也不会有问题。


## [876 Middle of the linked list](https://leetcode.com/problems/middle-of-the-linked-list/description/)

这个题目比较简单，但是提供了一个从物理世界借镜的视角。
S = V * T
...
同样的思路还可以*判断链表是否存在环*。如果存在环，不同移动速率的指针一定会在链表某处相遇

这里我还能在扩展一下，假设链表上有环，环长度为k。

* 慢指针速度为1.
* 快指针速度为f，每次移动f步长
* 慢指针在环内移动时，经过时间t后，位置是1 * t mod k
* 快指针在同一时间的位置是 f * t mod k

所以他们相遇就是 1 * t mod k == f * t mod k。
也就意味着（f-1） * t是k的倍数。
最小相遇时间，
也就是t = k / gcd(f-1, k)
...
刚刚我已经说了快慢指针在链表相遇点是一个gcd问题和环的大小有关，这个相遇点并不一定是
环的起点。
...
这样slow和fast指针就能在环起点重合。

``` c
struct ListNode *findCycleStart(struct ListNode *head){
...
## [Leetcode 160 两个链表是否相交](https://leetcode.com/problems/intersection-of-two-linked-lists/description/)
...
根据上文那个S = T *
V的逻辑我们只需要构造同样的速度和长度就可以在一个相同的节点相遇了。

## 后
这周是链表，下周是数组。

Sun Mar  2 08:46:58 PM CST 2025
